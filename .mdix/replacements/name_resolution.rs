...
impl<'a> Resolver<'a> {
    ...
    fn declare_builtins(&mut self) {
        ...
        // Builtin static namespaces (`Math.sqrt(x)`) — `Math` itself must
        // resolve as a name before `.sqrt` can be checked against it.
        for namespace in crate::builtins::BUILTIN_NAMESPACES {
            self.declare(namespace.to_string(), DefKind::Builtin, NO_SPAN, Visibility::Public);
        }
    }
