...
fn eval_call_with_receiver<'ast>(
    ...
    // Case 0: builtin static namespace — `Math.sqrt(x)` etc. Checked first
    // since `Math` will never be in `method_table` (that's exclusively
    // user-defined struct names) — a real name collision between a user
    // struct and a builtin namespace is caught at sema's declare-time,
    // not here.
    if let ExprKind::Ident(namespace) = &recv_expr.kind {
        if crate::builtins::is_builtin_namespace(namespace) {
            let run = crate::builtins::resolve_namespace_member(namespace, method_name)
                .ok_or_else(|| Signal::Panic(format!(
                    "no member '{}' in builtin namespace '{}'", method_name, namespace
                )))?;
...
fn eval_method_call(
    ...
    use crate::builtins::instance::{dict_methods, list_methods, string_methods, tuple_methods};

    match &receiver {
        // ── Built-in List methods ──────────────────────────────────
        Value::List(rc) => match method_name {
            "len"      => return Ok(list_methods::len(rc)),
            "push"     => return list_methods::push(rc, args),
            "pop"      => return Ok(list_methods::pop(rc)),
            "contains" => return list_methods::contains(rc, args),
            "first"    => return Ok(list_methods::first(rc)),
            "last"     => return Ok(list_methods::last(rc)),
            "is_empty" => return Ok(list_methods::is_empty(rc)),
            "reverse"  => return Ok(list_methods::reverse(rc)),
            _ => {}
        },
        ...
        Value::Str(s) => {
            ...
            match method_name {
                "len"         => return Ok(string_methods::len(&s)),
                "is_empty"    => return Ok(string_methods::is_empty(&s)),
                "to_upper"    => return Ok(string_methods::to_upper(&s)),
                "to_lower"    => return Ok(string_methods::to_lower(&s)),
                "trim"        => return Ok(string_methods::trim(&s)),
                "trim_start"  => return Ok(string_methods::trim_start(&s)),
                "trim_end"    => return Ok(string_methods::trim_end(&s)),
                "chars"       => return Ok(string_methods::chars(&s)),
                "contains"    => return string_methods::contains(&s, args),
                "starts_with" => return string_methods::starts_with(&s, args),
                "ends_with"   => return string_methods::ends_with(&s, args),
                "split"       => return string_methods::split(&s, args),
                "replace"     => return string_methods::replace(&s, args),
                _ => {}
            }
        }
        ...
        Value::Dict(rc) => match method_name {
            "len"          => return Ok(dict_methods::len(rc)),
            "is_empty"     => return Ok(dict_methods::is_empty(rc)),
            "contains_key" => return dict_methods::contains_key(rc, args),
            "keys"         => return Ok(dict_methods::keys(rc)),
            "values"       => return Ok(dict_methods::values(rc)),
            _ => {}
        },

        // ── Built-in Tuple methods ─────────────────────────────────
        Value::Tuple(elems) => match method_name {
            "len" => return Ok(tuple_methods::len(elems)),
            _ => {}
        },
